		...
		<section id="titlename" class="qnaBoard">
					...
					<form name="search" action="./BoardSearchList.do" method="post">
						<fieldset>
							...
							<label for="keyword"></label> <select name="keyfield"
								...
								<option value="title">
									<c:if test="${'name' == keyfield}">selected="selected"</c:if>>배우</option>
					...
					<c:when test="${listcount>0}">
						<thead>
							<tr>
								...
								<th scope="col" class="bbsTitle">배우</th>
						...
						<c:forEach var="actorList" items="${actorList}" varStatus="status">
							<tbody>
								<tr>
									<td><c:out value="${actorList.act_no}" /></td>
									<td><c:out value="${actorList.name}" /></td>
									<td><c:out value="${actorList.reg_date}" /><a href="./ActorDeleteProcess.jh?delNum=${actorList.act_no}"><button class="deleteBtn">삭제</button></a></td>
			...
			<div align="center">
					...
					<tbody>
						...
						<tr>
							<td colspan="5"><c:choose>
									...
									<c:otherwise>
										<a href="./ActorList.jh?page=<c:out value="${page-1}"/>">[이전]</a>&nbsp;
</c:otherwise>
									...
									<c:choose>
										<c:when test="${start eq page}"> [<c:out
												value="${start}" />] </c:when>
										<c:otherwise>
											<a href="./ActorList.jh?page=<c:out value="${start}" />">[<c:out
													value="${start}" />]
											</a>&nbsp;
</c:otherwise>
									</c:choose>
									...
									<c:otherwise>
										<a href="./ActorList.jh?page=<c:out value="${page+1}" />">[다음]</a>
									</c:otherwise>
								</c:choose></td>
						</tr>
					</tbody>
				...
				<div class="btnJoinAreb">
					...
					<a href="./ActorWrite.jh"><button type="button" value="button"
						 class="btnOk">
						글쓰기</button></a>
